...
import com.facebook.presto.operator.aggregation.stats.MergingDigest;
        ...
        implements AccumulatorStateSerializer<TDigestAndPercentileArrayState>
{
    ...
    public void deserialize(Block block, int index, TDigestAndPercentileArrayState state)
    {
        ...
        state.setDigest(MergingDigest.fromBytes(input.readSlice(length).toByteBuffer()));
        // TODO: How do we get the proper memory estimation? state.getDigest().estimatedInMemorySizeInBytes());
